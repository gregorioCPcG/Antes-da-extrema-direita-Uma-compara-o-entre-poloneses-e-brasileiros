...
urug <- subset(uru_s, select = c(Country, desconfia, direita, strong,abortion)) 

summary(urug) 

merge1 <- full_join(merge, urug)

write_xlsx(merge1,"D:/ATUALIZA_PASTA_d/Brasil_POL/analise extra/merge_comURUGUA.xlsx")

table(merge1$Country)

library(nnet) # pra regressão multinominal
merge1$Country <- as.factor(merge1$Country)
merge1$Country <- relevel(merge1$Country, "URU")
multi1<- multinom(Country ~ desconfia + direita + strong + abortion, data = merge1)
